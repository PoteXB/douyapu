...
setTimeout(function () {
    ...
    !function () {
        ...
        if (hitTb) {
            if ($("body").attr("dypSign159357") != 1) {
                ...
                cnzzAppend(function () {cnzzEvent("两个共存","检测");});
            }
        }
    }();
    //计算指定cookie剩余时间
    ...
    function start() {
                ...
                function startCou() {
                            ...
                            <div class="plugMid627-couQr">
                                ...
                                <div class="plugMid627-couQr-box">
                                    <div class="plugMid627-couQr-drop">
                                        ...
                                        <div class="">手淘扫码领券<br>商品<span>立减${b}元</span></div>
                                    </div>  
                                </div>
                            </div>
                        </div>`;
                    }   //有券的渲染
                    function noCouHtml(adData) {
                        return `<div><b>活动</b><span class="color" data-url="${adData[0].url}" data-moguDJ="无券广告1">${adData[0].name}</span></div>
                    ...
                    function getDanNewApi() {
                        chrome.extension.sendMessage({
                            ...
                            if (data.success && data.list && data.list.item && data.list.item.couponInfo && data.list.item.endTime && data.list.item.totalCount) {
                                ...
                                var html = hasCouHtml(data.finalPrice,amount,amountReq);
                                ...
                                $(".plugMid627-noCoupon").html(noCouHtml(adData));
                    ...
                    function setCoupon(list,type) {
                        ...
                        if (type == 'has') {
                            ...
                            var html = hasCouHtml(list.discountPrice,list.couponAmount / 100,"");
                            $(".plugMid627-hasCoupon").html(html);
                            $(".plugMid627-hasCoupon").show();
                            $(".plugMid627-couBack").click(function () {
        ...
        !function () {
            // var htmlVal = `<div id="plugBot627" style="z-index:9999999;position:fixed;left: 0;bottom: 0;">底部</div>`;
            // $('body').after(htmlVal);
        }()                                           //底部toolbar广告栏
    }
